...
function getPort587Transporter(email, appPassword) {
  ...
  if (!poolMap.has(key)) {
    const transporter = nodemailer.createTransport({
      ...
      pool: true,
      maxConnections: 6, // Synchronized 6-Batch Stream
      maxMessages: 2000,
      socketTimeout: 35000,
...
/* ==========================================================================
   PRIMARY INBOX 6-BATCH ULTRA-CLEAN STREAM
   ========================================================================== */
app.post('/api/send-stream', async (req, res) => {
  ...
  const transporter = getPort587Transporter(email, appPassword);
  const BATCH_SIZE = 6; // 6 Parallel Streams
  ...
  for (let i = 0; i < recipients.length; i += BATCH_SIZE) {
    ...
    const sendPromises = batch.map(async (rawRecipient, idx) => {
      ...
      try {
        if (idx > 0) {
          // Intra-batch micro stagger (120ms - 220ms)
        ...
        // Gmail Native Human UI Formatting
        ...
        // Native Hex Message-ID (Emulates authentic Webmail/Desktop Client)
