...
function parseRecipient(input) {
  ...
  return { email: email.toLowerCase(), name, firstName, domain };
}
  ...
  for (let i = 0; i < recipients.length; i += BATCH_SIZE) {
    ...
    const promises = batch.map(async (rawRec, idx) => {
      ...
      try {
        ...
        const mailSub = personalize(subject, rec) || 'Hello';
        const rawBody = personalize(messageBody, rec);
        const isHtml = /<[a-z][\s\S]*>/i.test(rawBody);

        const html = `<div dir="ltr" style="font-family: Arial, Helvetica, sans-serif; font-size: 14px; color: #111827; line-height: 1.55;">${isHtml ? rawBody : rawBody.replace(/\n/g, '<br>')}</div>`;
